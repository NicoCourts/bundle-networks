        ...
        for epoch in range(num_epochs):
            ...
            for batch, (bundle, base) in enumerate(loader):
                ...
                for (nbhd_idx, cover), basept in zip(bundle, base):
                    # downsample
                    num_pts = min(len(cover), num_pts)
                    pairs = sample(list(zip(list(cover), list(basept))), k=num_pts)
                    cover, basept = zip(*pairs)
                    cover, basept = torch.stack(cover).to(self.device), torch.stack(basept).to(self.device)

                    # Adversarial ground truths
                    valid = torch.ones([num_pts, 1], dtype=torch.float, requires_grad=False, device=self.device)
                    fake = torch.zeros([num_pts, 1], dtype=torch.float, requires_grad=False, device=self.device)

                    # Configure input
                    real_imgs = cover.to(self.device)

                    representatives = self.centers[nbhd_idx].unsqueeze(0).repeat(len(cover),1).to(self.device)
                    conditions = torch.cat([representatives, basept], dim=-1)
                    ...
                    z = torch.randn((num_pts, self.latent_dim), device=self.device)

                    # Generate a batch of images
                    # Here we condition on the basepoints & nbhd representative
                    gen_imgs = self.G(z, conditions)

                    # Loss measures generator's ability to fool the discriminator
                    validity = self.D(gen_imgs, conditions)
