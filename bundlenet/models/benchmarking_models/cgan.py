...
class CGAN(nn.Module):
    def __init__(self, cover_data, base_data, device='cpu', latent_dim=None):
        ...
        self.device = device
        self.G = self.Generator(self.latent_dim, self.total_dim, self.cond_dim)
        self.G.to(self.device)
        self.D = self.Discriminator(self.total_dim, self.cond_dim)
    ...
    def train_net(self,
        ...
        for epoch in range(num_epochs):
            ...
            for batch, (bundle, base) in enumerate(loader):
                ...
                conditions = torch.stack(base).to(self.device)
                ...
                z = torch.randn((len(real_imgs), self.latent_dim), device=self.device)
                
                # I had to change this here. We don't want to make any assumptions about the base space (that we're conditioning on).
                # Instead of randomly choosing integers, we will randomly sample the base space
                base_pts = sample(list(self.base_data), k=len(real_imgs))
                gen_labels = torch.stack(base_pts).to(self.device)
    ...
    def sample_from_fiber(self, basept, n=1):
        if n == 1:
            ...
            base_pts = basept.unsqueeze(0).repeat(2,1).to(self.device)
            ret = self.G(noise, base_pts).detach().cpu()[0]
            return ret
        else:
            z = torch.randn((n, self.latent_dim), device=self.device)
            base_pts = basept.unsqueeze(0).repeat(n,1).to(self.device)
            ret= self.G(z, base_pts).detach().cpu()
            return ret
